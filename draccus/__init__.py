# Minimal stub implementation of the draccus API used in Levanter tests.
...
class ChoiceRegistry:
    """Lightweight choice registry that mimics draccus' registry helpers."""
    ...
    def __init_subclass__(cls, choice_name: str | None = None, **kwargs):
        cls.discover_packages_path = kwargs.pop("discover_packages_path", getattr(cls, "discover_packages_path", None))
...
def _decode_inner(tp: Any, data: Any):
    """Best-effort recursive decoder for the subset of draccus features used in tests."""
...
def parse(config_class: type[T], config_file: str, args: Iterable[str] | None = None) -> T:
    """Minimal config file parser used in tests."""
